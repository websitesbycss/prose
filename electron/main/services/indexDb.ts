...
export function upsertIndex(row: IndexRow): void {
  getIndexDb()
    .prepare(`INSERT INTO documents (id, title, file_path, format, word_count, category_id, created_at, updated_at)
              VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    ...
    .run(row.id, row.title, row.file_path, row.format, row.word_count, row.category_id, row.created_at, row.updated_at)
}
